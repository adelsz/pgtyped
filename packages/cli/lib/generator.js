...
import { getTypes } from '@pgtyped/query';
...
import { TypeAllocator, TypeScope } from './types.js';
export var ProcessingMode;
(function (ProcessingMode) {
    ProcessingMode["SQL"] = "sql-file";
    ProcessingMode["TS"] = "query-file";
})(ProcessingMode || (ProcessingMode = {}));
const interfaceGen = (interfaceName, contents) => `export interface ${interfaceName} {
${contents}
}\n\n`;
export function escapeComment(comment) {
...
export const generateInterface = (interfaceName, fields) => {
...
export const generateTypeAlias = (typeName, alias) => `export type ${typeName} = ${alias};\n\n`;
export function queryToTypeDeclarations(parsedQuery, typeSource, types, config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const typeData = yield typeSource(queryData);
        ...
        const hasAnonymousColumns = !typeError &&
            typeData.returnTypes.some(({ returnName }) => returnName === '?column?');
        if (typeError || hasAnonymousColumns) {
            // tslint:disable:no-console
            if (typeError) {
                console.error('Error in query. Details: %o', typeData);
                if (config.failOnError) {
                    throw new Error(`Query "${queryName}" is invalid. Can't generate types.`);
                }
            }
            else {
                console.error(`Query '${queryName}' is invalid. Query contains an anonymous column. Consider giving the column an explicit name.`);
            }
            let explanation = '';
            if (hasAnonymousColumns) {
                explanation = `Query contains an anonymous column. Consider giving the column an explicit name.`;
            }
            const returnInterface = generateTypeAlias(`${interfacePrefix}${interfaceName}Result`, 'never');
            const paramInterface = generateTypeAlias(`${interfacePrefix}${interfaceName}Params`, 'never');
        ...
        for (const param of paramMetadata.mapping) {
            if (param.type === ParameterTransform.Scalar ||
                ...
                let fieldType = Object.values(param.dict)
                    .map((p) => {
                    const paramType = types.use(params[p.assignedIndex - 1], TypeScope.Parameter);
                    return p.required
                        ? `    ${p.name}: ${paramType}`
...
export function generateTypedecsFromFile(contents, fileName, connection, transform, types, config) {
    return __awaiter(this, void 0, void 0, function* () {
        const typedQueries = [];
        const interfacePrefix = config.hungarianNotation ? 'I' : '';
        const typeSource = (query) => getTypes(query, connection);
        const { queries, events } = transform.mode === 'sql'
        ...
        for (const queryAST of queries) {
            ...
            if (transform.mode === 'sql') {
                ...
                const result = yield queryToTypeDeclarations({ ast: sqlQueryAST, mode: ProcessingMode.SQL }, typeSource, types, config);
                typedQuery = {
                    mode: 'sql',
                    query: {
                ...
                typedQuery = {
                    ...
                    query: {
                        ...
                        ast: tsQueryAST,
                        queryTypeAlias: `${interfacePrefix}${pascalCase(tsQueryAST.name)}Query`,
                    },
                    typeDeclaration: result,
                };
            }
...
export function generateDeclarations(typeDecs) {
    ...
    for (const typeDec of typeDecs) {
        ...
        const queryPP = typeDec.query.ast.statement.body
            ...
            .map((s) => ' * ' + s)
            .join('\n');
        typeDeclarations += `const ${typeDec.query.name}IR: any = ${JSON.stringify(typeDec.query.ir)};\n\n`;
        typeDeclarations +=
            `/**\n` +
...
export function generateDeclarationFile(typeDecSet) {
    ...
    content += TypeAllocator.typeDefinitionDeclarations(typeDecSet.fileName, typeDecSet.typeDefinitions);
    content += '\n';
    content += generateDeclarations(typeDecSet.typedQueries);
    return content;
}
export function genTypedSQLOverloadFunctions(functionName, typedQueries) {
    return typedQueries
        .map((typeDec) => `export function ${functionName}(s: \`${typeDec.query.ast.text}\`): ReturnType<typeof sourceSql<${typeDec.query.queryTypeAlias}>>;`)
        .filter((s) => s)
        .join('\n');
}
